...
var ck_id =  /^[0-9]/;
            ...
            if(!ck_id.test(uid))
            {
                ...
                alert("ROLL NUMBER can only contain numeric characters and hypehns(-)");
return false;
            }
var lnam =  /^[A-Za-z]/;
...
*<input type="text" name="user" placeholder="Rollnumber"><br />
...
if(!empty($_POST['user']) && !empty($_POST['pass']) && !empty($_POST['repass']) && !empty($_POST['fname']) && !empty($_POST['email']) && !empty($_POST['month']) && !empty($_POST['year']) && !empty($_POST['day']) && !empty($_POST['gender']) && !empty($_POST['branch'])) {
	...
	if($numrows==0)
	{
	$sql="INSERT INTO login(username,password,fname,lname,email,gender,dob,branch,git,bio) VALUES('$user','$pass','$fn','$ln','$em','$gender','$date','$branch','$git','$bio')";
